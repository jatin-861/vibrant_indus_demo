...
import { Download, FileText, TrendingUp, DollarSign, CheckCircle, Clock, XCircle } from 'lucide-react';
import * as XLSX from 'xlsx';
import type { Invoice, Payment, Shade, Owner, SystemSettings } from '../types';
...
type ReportType = 'monthly_collection' | 'additional_charges' | 'payment_performance';

const formatCurrency = (val: number) =>
  new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(val);

// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const ReportsView: React.FC<ReportsViewProps> = ({ invoices, payments: _payments, shades, owners: _owners, settings }) => {
  ...
  const additionalData = useMemo(() => {
    const rows: { invoiceId: string; shadeId: string; shadeLabel: string; transferFee: number; otherName: string; otherCharge: number; date: string }[] = [];
    activeInvoices.forEach(inv => {
      const shade = shades.find(s => s.id === inv.shadeId);
      const shadeLabel = shade ? `${shade.block}-${shade.floor}` : inv.shadeId;
      if (inv.transferFee > 0 || inv.otherMaintenanceCharge > 0) {
        rows.push({
          invoiceId: inv.id,
          shadeId: inv.shadeId,
          shadeLabel,
  ...
  const exportToPDF = () => {
    ...
    const headers = activeReport === 'monthly_collection'
      ...
      : activeReport === 'additional_charges'
      ? '<th>Invoice</th><th>Shade</th><th>Date</th><th>Transfer Fee</th><th>Other Name</th><th>Other Amount</th>'
        ...
        <div>
          <div className="stats-grid" style={{ marginBottom: '20px' }}>
            ...
            <div className="stat-card">
              ...
              <div className="stat-value" style={{ color: 'var(--primary)' }}>{formatCurrency(totalTransfer + totalOther)}</div>
            </div>
          </div>
          ...
          <div className="table-container">
            <table className="data-table">
              <thead>
                <tr><th>Invoice</th><th>Shade</th><th>Date</th><th>Transfer Fee</th><th>Other</th><th>Other Type</th></tr>
              </thead>
              <tbody>
                {additionalData.length === 0 ? (
                  <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>No additional charges found</td></tr>
                ) : additionalData.map(r => (
                  <tr key={r.invoiceId}>
